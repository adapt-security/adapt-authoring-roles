 ...
 */
export default class RolesModule extends AbstractApiModule {
  /** @override */
  async init() {
    await super.init();
    try {
  ...
  async initDefaultRoles() {
    ...
    users.insertHook.tap(data => {
      if(!data.roles || !data.roles.length) {
        data.roles = rolesForAuth[data.authTypes] || rolesforAll || [];
      }
    });
  }
  /** @override */

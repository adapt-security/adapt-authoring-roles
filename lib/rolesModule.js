...
class RolesModule extends AbstractApiModule {
  ...
  async init() {
    ...
    if(this.getConfig('defaultRoles').length) {
  ...
  */
  async initConfigRoles() {
    return Promise.allSettled(this.getConfig('roleDefinitions').map(async r => {
      ...
      if(doc) {
        try {
          await this.replace({ _id: doc._id }, r);
  ...
  */
  async initDefaultRoles() {
    ...
    const query = shortNames.length > 0 ? { $or: shortNames } : {};
    const defaultRoles = (await this.find(query)).map(r => r._id.toString());
